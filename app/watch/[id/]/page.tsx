...
export default function WatchPage() {
  ...
  return (
    <main className="flex h-screen bg-[#120d09] overflow-hidden text-[#f1e4da] selection:bg-[#bc6746]/20 font-sans">
      
      {/* Cinematic Sidebar - Explorer */}
      <aside 
        className={`fixed inset-y-0 left-0  z-[100] w-[340px] bg-[#1a1512] border-r border-white/5 transition-all duration-700 ease-in-out lg:relative lg:translate-x-0 ${
      ...
      >
         <div className="flex flex-col h-full">
               ...
               {course.course_sections?.map((section: any, sIdx: number) => (
                  <div key={section.id} className="space-y-4">
                     <div className="flex items-baseline gap-4 px-2">
                        <span className="text-[12px] font-black uppercase tracking-widest text-[#bc6746] opacity-30 italic">Module {String(sIdx + 1).padStart(2, '0')}</span>
                        <h3 className="text-[13px] font-bold text-white/80 lowercase tracking-tight first-letter:uppercase">{section.title}</h3>
                     </div>
                           ...
                           return (
                              ...
                              >
                                 <div className={`shrink-0 w-10 h-10 rounded-xl flex items-center justify-center transition-all ${isActive ? 'bg-white/20' : 'bg-white/5 group-hover:scale-110 shadow-lg'}`}>
                                    {isActive ? <SkipForward className="w-5 h-5 fill-current" /> : <MonitorPlay className="w-5 h-5 text-[#bc6746]" />}
                                 </div>
                                 <div className="flex-1 overflow-hidden">
                                    <div className="flex items-center gap-2 mb-1.5">
                                        <p className={`text-[13px] font-bold truncate leading-none ${isActive ? 'text-white' : 'text-white/70'}`}>
                                            {lesson.title}
                                        </p>
            ...
            {/* Profile Footer */}
            <div className="p-8 bg-[#1a1512] border-t border-white/5 flex items-center justify-between">
               <div className="flex items-center gap-4">
                  <div className="relative">
                      <div className="w-12 h-12 rounded-2xl bg-[#bc6746] text-white flex items-center justify-center font-serif text-xl border-2 border-white/10 shadow-2xl rotate-3">
                         {student?.name?.charAt(0)}
                      </div>
                      <div className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-emerald-500 border-2 border-[#1a1512]" />
                  </div>
                  <div>
                     <p className="text-[11px] font-black uppercase tracking-[0.1em] text-white/90">{student?.name}</p>
                     <p className="text-[9px] font-bold text-[#bc6746] uppercase tracking-widest">Sanctuary Guarded</p>
                  </div>
               </div>
            </div>
         </div>
      ...
      <section className="flex-1 flex flex-col h-full bg-[#120d09] relative shadow-[inset_40px_0_100px_-20px_rgba(0,0,0,0.8)]">
         ...
         {/* Cinematic Stage */}
         <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar pt-0">
            {/* The Cinema Container */}
            <div className={`relative transition-all duration-1000 ease-in-out ${isTheaterMode ? 'px-0 pt-0' : 'px-10 pt-28 md:px-20'}`}>
                <div className={`max-w-7xl mx-auto transition-all duration-1000 ${isTheaterMode ? 'max-w-none' : ''}`}>
                    <div className={`relative aspect-video bg-black overflow-hidden group transition-all duration-1000 select-none ${isTheaterMode ? '' : 'rounded-[40px] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.8)] border border-white/5'}`}>
                        {currentLesson && embedUrl ? (
                            <iframe 
                                src={embedUrl}
                                loading="lazy"
                                style={{ border: 0, position: 'relative', top: 0, height: '100%', width: '100%' }}
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center space-y-4">
                                ...
                                <p className="text-white/20 text-[10px] font-black uppercase tracking-[0.5em]">
                                  {currentLesson ? "Video Access Unavailable" : "Establishing Connection"}
                                </p>
                            </div>
                        )}
                        ...
                        <div className="absolute top-8 right-8 opacity-0 group-hover:opacity-100 transition-opacity">
                            <div className="px-5 py-2 rounded-full bg-black/60 backdrop-blur-xl border border-white/10 text-[10px] font-black uppercase tracking-widest">
                                UHD 4K Streaming
                            </div>
                        </div>
                    </div>
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-16">
                     ...
                     <div className="shrink-0 flex flex-col gap-6 w-full lg:w-72">
                        <button 
                           ...
                           className={`flex items-center justify-center gap-4 py-6 rounded-[32px] text-[12px] font-black uppercase tracking-[0.2em] shadow-[0_30px_60px_-10px_rgba(188,103,70,0.3)] transition-all transform active:scale-95 group ${
                        ...
                        >
                           {completing ? (
                               ...
                               <>
                                 ...
                                 <CheckCircle2 className={`w-5 h-5 ${completedLessonIds.includes(currentLesson.id) ? 'text-white' : 'group-hover:rotate-12 transition-transform'}`} />
                               </>
                           )}
                        </button>
                        ...
                        {nextLesson && (
                            <button 
                                ...
                                className="flex items-center justify-center gap-4 py-6 border border-white/10 bg-white/5 hover:bg-white/10 text-white/80 rounded-[32px] text-[10px] font-black uppercase tracking-[0.2em] transition-all group"
                            >
                                <span>Up Next: {nextLesson.title}</span>
                                <SkipForward className="w-4 h-4 opacity-40 group-hover:opacity-100" />
                            </button>
                        )}
                     </div>
                  </div>
